import { type CanActivate, type ExecutionContext, Injectable, NotFoundException, UnauthorizedException } from '@nestjs/common'
...
export class SessionAuthGuard implements CanActivate {
	...
	public async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		await this.redisService.expire(`user_sessions:${currentSession.id}`, 7 * 24 * 60 * 60)
...
export class OptionalSessionAuthGuard implements CanActivate {
	...
	async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		if (!currentSession) return true // не авторизований = ghost
