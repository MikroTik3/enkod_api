...
export class CourseService {
	...
	public async getBySlug(slug: string) {
		...
		if (!course?.isPublished) throw new NotFoundException('Course not found')
		...
		await this.redisService.set(`courses:${course.slug}`, JSON.stringify(dto), 'EX', 10 * 60)
	...
	public async generateDownloadLink(id: string, user: User) {
		...
		if (!course?.attachment) throw new NotFoundException('Course not found')

		const token = randomBytes(16).toString('hex')

		await this.redisService.set(`course:download:${token}`, JSON.stringify({ courseId: course.id, userId: user.id }), 'EX', 60 * 5)
	...
	public async resolveDownloadToken(token: string, ip: string, userAgent: string) {
