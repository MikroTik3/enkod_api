...
import { Authorized, ClientIp, SubscriptionOnly, UserAgent } from '@/shared/decorators'
import { Authorization } from '@/shared/decorators/auth.decorator'

import { CourseService } from './course.service'
import { CourseResponse } from './dto/course.dto'
import { CoursesResponse } from './dto/courses.dto'
import { CreateCourseRequest, CreateCourseResponse } from './dto/create-course.dto'
...
export class CourseController {
	...
	@HttpCode(HttpStatus.OK)
	public async generateDownloadLink(@Param('id') id: string, @Authorized() user: User) {
		return await this.courseService.generateDownloadLink(id, user)
	}

	@ApiOperation({
		summary: 'Resolve download token',
		description: 'Resolves a download token to stream the course attachment.'
	})
	@ApiOkResponse({
		description: 'The requested course file is streamed as a ZIP attachment.'
	})
	@Get('download/:token')
	@HttpCode(HttpStatus.OK)
	public async resolveDownloadToken(@Param('token') token: string, @ClientIp() ip: string, @UserAgent() userAgent: string, @Res() res: Response) {
		const course = await this.courseService.resolveDownloadToken(token, ip, userAgent)

		try {
			const { stream, contentType } = await this.courseService.fetchAttachmentStream(course.attachment)

			res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(course.title)}.zip"`)
