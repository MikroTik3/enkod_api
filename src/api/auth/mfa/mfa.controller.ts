...
import { ApiBody, ApiHeader, ApiOkResponse, ApiOperation, ApiTags, getSchemaPath } from '@nestjs/swagger'
...
import { MfaRecoveryRequest, MfaStatusResponse, MfaTotpRequest, TotpDisableRequest, TotpEnableRequest, TotpGenerateSecretResponse } from './dto'
...
export class MfaController {
	...
	@ApiOperation({
		...
		description: 'Verify the MFA ticket for the authenticated user, either by TOTP code or recovery code.'
	})
	@ApiBody({
		schema: {
			anyOf: [{ $ref: getSchemaPath(MfaTotpRequest) }, { $ref: getSchemaPath(MfaRecoveryRequest) }]
		}
	})
	@ApiOkResponse({
	...
	@Post('verify')
	@HttpCode(HttpStatus.OK)
	public async verify(@Body() dto: MfaTotpRequest | MfaRecoveryRequest, @ClientIp() ip: string, @UserAgent() userAgent: string) {
