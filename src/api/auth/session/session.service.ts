...
export class SessionService {
	...
	public async login(dto: LoginRequest, ip: string, userAgent: string) {
		...
		if (!user) throw new NotFoundException('Неправильний логін або пароль')

		if (!user.password) {
			throw new BadRequestException('Цей аккаунт зареєстрований через зовнішній провайдер (Google або Facebook)')
		}

		const isValidPassword = await verify(user.password, password)

		if (!isValidPassword) throw new NotFoundException('Неправильний логін або пароль')
	...
	public async loginAdmin(dto: LoginRequest, ip: string, userAgent: string) {
		...
		if (!admin) throw new NotFoundException('Неправильний логін або пароль')
	...
	public async removeAll(user: User, token: string) {
		...
		if (sessions.length > 0) {
			await Promise.all([...sessions.map(key => this.redisService.del(key)), ...userSessions.map(key => this.redisService.del(key))])
		}
