...
import { ApiExtraModels, ApiOkResponse, ApiOperation, ApiTags, refs } from '@nestjs/swagger'
...
import { LoginMfaResponse, LoginRequest, LoginSessionResponse, SessionResponse } from './dto'
...
export class SessionController {
	...
	public async login(@Body() dto: LoginRequest, @ClientIp() ip: string, @UserAgent() userAgent: string) {
	...
	@TurnstileCaptcha()
	@HttpCode(HttpStatus.OK)
	public async loginAdmin(@Body() dto: LoginRequest, @ClientIp() ip: string, @UserAgent() userAgent: string) {
	...
	@Get('all')
	@HttpCode(HttpStatus.OK)
	public async getSessions(@Authorized() user: User, @Headers('x-session-token') token: string) {
	...
	@Delete('all')
	@HttpCode(HttpStatus.OK)
	public async removeAll(@Authorized() user: User, @Headers('x-session-token') token: string) {
	...
	@Delete(':id')
	@HttpCode(HttpStatus.OK)
	public async revoke(@Param('id') id: string, @Headers('x-session-token') token: string) {
