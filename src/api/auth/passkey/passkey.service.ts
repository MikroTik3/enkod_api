...
export class PasskeyService {
	...
	public async fetchPasskeys(user: User) {
		...
		if (!mfa?.passkeys) {
			throw new NotFoundException('Багатофакторна автентифікація не ввімкнена')
		}
	...
	public async verifyRegistration(user: User, deviceName: string, attestationResponse: any, userAgent: string, ip: string) {
		const mfa = await this.prismaService.multiFactorAuthentication.findUnique({
			where: {
				userId: user.id
			}
		...
		if (!mfa?.currentChallenge) throw new BadRequestException('No challenge found')
		...
		if (!verification.verified) throw new BadRequestException('Verification failed')
	...
	public async generateAuthenticationOptions(userId: string) {
		...
		if (!mfa?.passkeys.length) throw new NotFoundException('Немає зареєстрованих ключів доступу')
		...
		await this.redisService.set(`passkey_challenge:${userId}`, options.challenge, 'EX', 300)

		return options
	}

	public async verifyAuthentication(user: User, assertionResponse: any) {
		const expectedChallenge = await this.redisService.get(`passkey_challenge:${user.id}`)
		if (!expectedChallenge) throw new BadRequestException('Challenge не найден или истёк')
		...
		const passkey = mfa.passkeys.find(pk => pk.credentialId === assertionResponse.id)

		if (!passkey) throw new BadRequestException('Ключ не зареєстрований')
		...
		const verification = await verifyAuthenticationResponse({
			...
			credential: {
				...
				publicKey: new Uint8Array(base64url.toBuffer(passkey.publicKey)),
				counter: passkey.counter ?? 0
			}
		})

		if (!verification.verified) throw new UnauthorizedException('Аутентифікація ключа не пройдена')
	...
	public async delete(id: string, user: User) {
		...
		if (passkey?.mfa.userId !== user.id) throw new NotFoundException('Ключ доступу не знайдено')
		...
		const remainingPasskeys = passkey.mfa.passkeys.filter(pk => pk.id !== passkey.id)

		if (passkey.mfa.totp?.status !== TotpStatus.ENABLED && remainingPasskeys.length === 0 && passkey.mfa.recoveryCodes?.length) {
			await this.prismaService.multiFactorAuthentication.update({
				where: {
					id: passkey.mfa.id
				},
