import { Body, Controller, Delete, Get, HttpCode, HttpStatus, Ip, Param, Post } from '@nestjs/common'
...
export class PasskeyController {
	...
	@ApiOperation({
		...
		description: 'Retrieve the list of registered passkeys for the authenticated user.'
	})
	...
	@HttpCode(HttpStatus.OK)
	public async verifyRegistration(@Authorized() user: User, @Body() body: { deviceName: string; attestationResponse: any }, @UserAgent() userAgent: string, @Ip() ip: string) {
		return this.passkeyService.verifyRegistration(user, body.deviceName, body.attestationResponse, userAgent, ip)
	}

	@Post('login/options')
	@HttpCode(HttpStatus.OK)
	public async generateAuthenticationOptions(@Body() dto: { userId: string }) {
