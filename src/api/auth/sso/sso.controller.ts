...
import { Authorization, Authorized, ClientIp, UserAgent } from '@/shared/decorators'
import { ProviderGuard } from '@/shared/guards'

import { SsoConnectResponse, SsoLoginRequest, SsoLoginResponse, SsoStatusResponse, TelegramAuthRequest, TelegramAuthResponse } from './dto'
...
export class SsoController {
	...
	@ApiOperation({
		...
		description: 'Returns the status of external accounts (e.g., Google, GitHub) linked to the current user.'
	})
	@ApiOkResponse({
	...
	@ApiOperation({
		...
		description: 'Returns a list of available external authentication providers (e.g., Google, GitHub, Discord, Telegram).'
	})
	@ApiOkResponse({
		schema: {
			type: 'array',
	...
	@ApiOperation({
		...
		description: 'Generates the login URL for the specified external provider (e.g., Google, GitHub).'
	})
	@ApiParam({
		name: 'provider',
	...
	public async getLoginUrl(@Param('provider') provider: string, @Body() dto: SsoLoginRequest) {
		...
		return {
			url: provider === 'telegram' ? this.ssoService.getTelegramAuthUrl('login') : providerInstance.getAuthUrl(state)
		}
	}
	...
	@ApiOperation({
		...
		description: 'Unlink an external account (e.g., Google, GitHub) from the current user.'
	})
	@ApiParam({
		name: 'provider',
		description: 'The external provider (google, github, etc.)',
	...
	@Delete(':provider')
	@HttpCode(HttpStatus.OK)
	public async unlink(@Param('provider') provider: AllowedProvider, @Authorized() user: User) {
