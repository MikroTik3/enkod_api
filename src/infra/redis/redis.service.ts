...
export class RedisService extends Redis implements OnModuleInit, OnModuleDestroy {
	...
	public async createSession(user: User, options: CreateSessionOptions) {
		...
		if (visitorId && requestId) {
			visitorHistory = await this.fingerprintService.getVisitorHistory(visitorId)
		}

		this.parser.setUA(userAgent ?? '')
		const uaResult = this.parser.getResult()

		const visits = Array.isArray(visitorHistory?.visits) ? visitorHistory.visits : []
		...
		await this.set(`user_sessions:${session.id}`, JSON.stringify(userSession), 'EX', 7 * 24 * 60 * 60)
	...
	public async getUserSession(sessionId: string): Promise<UserSession | null> {
		try {
			const raw = await this.get(`user_sessions:${sessionId}`)

			if (!raw) {
				this.logger.warn(`UserSession not found for sessionId: ${sessionId}`)
				return null
			}
			...
			if (!data?.id || !data.sessionId) {
				this.logger.warn(`Invalid UserSession data for sessionId: ${sessionId}`)
				return null
			}

			return data
		} catch (error) {
			this.logger.error(`Failed to get UserSession for sessionId: ${sessionId}`, error)
			throw error
		}
	}
